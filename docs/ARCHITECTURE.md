...
Infi mirrors LaReview's layered shape while replacing diff review concepts with stock analysis concepts.
...
4. The ACP session mounts a stdio MCP server named `infi-analysis`.
5. The system prompt instructs the agent to research with all available tools but submit output only through Infi tools.
