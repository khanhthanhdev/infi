...
fn config_path() -> PathBuf {
    if let Ok(path) = std::env::var("INFI_CONFIG_PATH") {
    ...
    #[cfg(target_os = "windows")]
    {
        if let Some(appdata) = std::env::var_os("APPDATA") {
            return PathBuf::from(appdata).join("Infi").join("config.json");
        }
    }
    ...
    #[cfg(target_os = "linux")]
    {
        if let Some(xdg) = std::env::var_os("XDG_CONFIG_HOME") {
            return PathBuf::from(xdg).join("infi").join("config.json");
        }
        if let Some(home) = dirs::home_dir() {
            return home.join(".config").join("infi").join("config.json");
        }
    }
...
mod tests {
    ...
    // INFI_CONFIG_PATH is process-global; serialize tests that touch it.
    ...
    impl EnvGuard {
        fn set(path: &std::path::Path) -> Self {
            ...
            let previous = std::env::var("INFI_CONFIG_PATH").ok();
            unsafe {
                std::env::set_var("INFI_CONFIG_PATH", path);
            }
            Self {
                _lock: lock,
                previous,
    ...
    impl Drop for EnvGuard {
        fn drop(&mut self) {
            unsafe {
                match &self.previous {
                    Some(value) => std::env::set_var("INFI_CONFIG_PATH", value),
                    None => std::env::remove_var("INFI_CONFIG_PATH"),
                }
            }
        }
    }
