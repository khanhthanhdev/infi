...
impl InfiClient {
    ...
    fn handle_extension_payload(&self, method: &str) -> bool {
        if matches!(method, "infi/finalize_analysis" | "finalize_analysis") {
            self.mark_finalization_received();
            return true;
        }
        INFI_TOOLS.iter().any(|tool| method.contains(tool))
    }
}
...
#[async_trait(?Send)]
impl agent_client_protocol::Client for InfiClient {
...
mod tests {
    ...
    fn make_client() -> (InfiClient, mpsc::UnboundedReceiver<ProgressEventPayload>) {
        let (tx, rx) = mpsc::unbounded_channel();
        (InfiClient::new(Some(tx)), rx)
    }
    ...
    async fn terminal_toolcall_for_submit_source_fires_source_submitted() {
        ...
        client
            .session_notification(notify(SessionUpdate::ToolCall(
                ToolCall::new("call-submit".to_string(), "mcp: infi submit_source(...)")
                    .kind(ToolKind::Other)
                    .status(ToolCallStatus::Completed),
            )))
            .await
