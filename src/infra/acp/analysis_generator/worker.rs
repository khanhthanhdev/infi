...
pub struct GenerateAnalysisInput {
    ...
    /// keyed by provider id. Injected as `INFI_SRC_KEY_<ID_UPPER>` on the
...
async fn generate_with_acp_inner(input: GenerateAnalysisInput) -> Result<GenerateAnalysisResult> {
    ...
    let client = InfiClient::new(progress_tx.clone());
    ...
    let result = async {
        connection
            .initialize(
                InitializeRequest::new(ProtocolVersion::V1)
                    .client_info(Implementation::new("infi", env!("CARGO_PKG_VERSION")))
        ...
        let current_exe = std::env::current_exe().unwrap_or_else(|_| PathBuf::from("infi"));
        ...
        for (id, value) in &source_keys {
            env_vars.push(EnvVariable::new(
                format!("INFI_SRC_KEY_{}", id.to_uppercase()),
                value.clone(),
            ));
        }

        let mcp_servers = vec![McpServer::Stdio(
            McpServerStdio::new("infi-analysis", mcp_path.clone())
                .args(vec![
