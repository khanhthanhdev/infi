...
pub struct ServerConfig {
    ...
    /// Provider id → API key. Populated from `INFI_SRC_KEY_<ID>` env
...
impl ServerConfig {
        ...
        I: IntoIterator<Item = String>,
    {
        ...
        if let Ok(path) = std::env::var("INFI_ANALYSIS_CONTEXT") {
            config.run_context = Some(PathBuf::from(path));
        }
        if let Ok(path) = std::env::var("INFI_DB_PATH") {
            config.db_path = Some(PathBuf::from(path));
        }
        for (name, value) in std::env::vars() {
            if let Some(suffix) = name.strip_prefix("INFI_SRC_KEY_")
                && !suffix.is_empty()
                && !value.is_empty()
            {
