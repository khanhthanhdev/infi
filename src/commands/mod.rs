...
pub async fn publish_analysis_html(
    ...
    let endpoint =
        std::env::var("INFI_PUBLISH_URL").unwrap_or_else(|_| DEFAULT_PUBLISH_URL.to_string());
...
mod tests {
    ...
    fn safe_filename_sanitises_title() {
        ...
        assert_eq!(safe_filename(""), "infi-report.html");
        assert_eq!(safe_filename("  "), "infi-report.html");
    }
    ...
    fn standalone_html_escapes_script_close_and_html_comment() {
        ...
        assert!(
            html.contains("INFIPAYLOAD <\\/script><\\!--"),
            "expected escaped sequence in html"
        );
        assert!(!html.contains("INFIPAYLOAD </script>"));
        assert!(!html.contains("INFIPAYLOAD <!--"));
    }
