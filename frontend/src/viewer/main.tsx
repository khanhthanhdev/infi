...
declare global {
  interface Window {
    __INFI_REPORT__?: AnalysisReport | string;
  }
}

// biome-ignore lint/suspicious/noExplicitAny: placeholder swapped at build/export time
window.__INFI_REPORT__ = "__INFI_REPORT_JSON__" as any;

function readEmbeddedReport(): AnalysisReport | null {
  const raw = window.__INFI_REPORT__;
  if (!raw || raw === "__INFI_REPORT_JSON__") return null;
